...
The projectroot is the folder where the project information and database are stored; it is displayed in the Tahoma browser as one of the main folders with the related path in brackets.

The default value is ``C:\Tahoma stuff\projects``  on Windows and ``/Applications/Tahoma/Tahoma stuff/projects``  on Macintosh.
...
It is also possible to define more than one projectroot if needed: in this case projectroot folders will be visible in the Tahoma browser, each with the related path in brackets.
...
.. tip:: **To set the projectroot on Windows:**
    ...
    6. Restart Tahoma to make the change effective.
...
.. tip:: **To add a projectroot on Windows:**
    ...
    4. Restart Tahoma to make the change effective.

.. tip:: **To set the projectroot on Macintosh:**

    1. Right-click the Tahoma icon in the ``Applications/Tahoma`` folder, and choose Show Package Contents from the menu that opens.
    ...
    ``TOONZPROJECTS=”/Applications/Tahoma/Tahoma stuff/projects”`` 
...
.. tip:: **To add a projectroot on Macintosh:**
    ...
    2. Change the path in inverted commas by adding a new path for the new projectroot, separated by a semicolon (e.g. ``/Applications/Tahoma/Tahoma stuff/production01`` ``;`` ``/Applications/Tahoma/Tahoma stuff/production02`` ). 
...
- **+drawings** is the folder where Tahoma cleaned up drawings, drawings made directly in Tahoma and imported Tahoma drawings are saved (TLV and PLI files).

- **+scenes** is the folder where the Tahoma scenes are saved (TNZ files).
...
The labels of the default folders you can find in the project settings (i.e. **+drawings**, **+extras**, etc.) can be used as aliases in Tahoma. For example if some material is retrieved from the extras default folder, its loading path will contain the alias **+extras**. They can be also used when you need to specify a saving path, for example in the Path field of the New Level dialog.
...
For example if **+drawings** is ``$scenepath\mydrawings,`` and you save the scene scene01 in ``+scenes\seq01`` , Tahoma drawn or cleaned up levels for that scene will be saved in ``seq01\scene01\mydrawings`` ; when you save the scene scene02, in ``seq01\scene02\mydrawings`` ; and so on. 
...
A second example: if **+drawings** is ``mydrawings\$scenepath,`` when you save the scene scene01 in ``+scenes\seq01`` , Tahoma drawn or cleaned up levels for that scene will be saved in ``mydrawings\seq01\scene01`` ; when you save the scene scene02, in ``mydrawings\seq01\scene02`` ; and so on. 
...
For example if **+drawings** is ``mydrawings`` , and you save the scene scene01 in ``+scenes\seq01`` , Tahoma drawn or cleaned up levels for that scene will be saved in ``mydrawings\seq01\scene01`` ; if you save the scene scene02 in ``+scenes\seq01`` , Tahoma drawn or cleaned up levels for that scene will be saved in ``mydrawings\seq01\scene02`` ; and so on.

 |append_scenepath| 

This allows Tahoma to create scene-related folders automatically when saving Tahoma files or importing external files, while allowing the use of the generic aliases **+drawings**, **+inputs** and **+extras**, no matter where levels are loaded from within the current project.
...
-------------------------
The project browser is available at the end of the file browser tree. It lists all of the projects that have been created, whose actual location is in the projectroot that was defined in the initial installation of Tahoma. The projectroot value is displayed in brackets.
