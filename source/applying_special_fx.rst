...
Shaders
'''''''
Tahoma enables support for effects rendered through hardware-accelerated pixelshaders. Shaders are simple programs compiled by a graphics processing unit that harness the massively parallel architecture of modern graphics devices, to execute with extreme speed. Tahoma's shader effects are written in the OpenGL Shading Language (GLSL), any GPU within the past 10 years will work fine, and are located in ``Tahoma stuff/library/shaders``. Tahoma2D baseline support is OpenGL 2.0, but more recent versions up till OpenGL 3.30 should work with minimal changes. It's recommended to follow GLSL ES, which is based off GLSL 3.2 and WebGL 2.0 uses that as well. `The Book Of Shaders <https://thebookofshaders.com/>`_, `Shadertoy <https://www.shadertoy.com/>`_, `the khronos OpenGL ES 3.2 API listings <https://registry.khronos.org/OpenGL-Refpages/es3/>`_, and the convenient `reference card PDF <https://www.khronos.org/files/opengles32-quick-reference-card.pdf>`_ will be highly useful in learning or refreshing knowledge in how shaders work. See also the readme.txt file for further details on editing or creating new shader effects, because it will list special variables used only with Tahoma.
